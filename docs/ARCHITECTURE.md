...
`Program` is a builder value containing instructions, initial bytes, labels and entry metadata. Published programs are immutable. Callers using the shared-pointer constructor must not retain a mutable alias to the same program. Machine copies have private mutable state and share code; they never expose references to another machine's memory.
...
The protocol favors command responsiveness over maximum interpreter throughput; an instruction checks the command queue once. Benchmark before introducing batches. Program copying has been removed from the execution hot path. Snapshots intentionally copy memory for isolation; zero-copy references to live mutable state are not an optimization.
